...
func (sc *ShardedCache) SetWithOptions(key string, value interface{}, size int64, ttl time.Duration, persistent bool) error {
	shard := sc.getShard(key)
	
	shard.mu.Lock()
	defer shard.mu.Unlock()
	
	return shard.cache.SetWithOptions(key, value, size, ttl, persistent)
}
