...
func parseSize(sizeStr string) (int64, error) {
	...
	// Simple size parsing - could be more sophisticated
	switch {
	case len(sizeStr) >= 2 && sizeStr[len(sizeStr)-2:] == "MB":
		var size int64
		if _, err := fmt.Sscanf(sizeStr, "%dMB", &size); err != nil {
			return 0, err
		}
	...
	case len(sizeStr) >= 2 && sizeStr[len(sizeStr)-2:] == "GB":
		var size int64
		if _, err := fmt.Sscanf(sizeStr, "%dGB", &size); err != nil {
			return 0, err
		}
		return size * 1024 * 1024 * 1024, nil
		
	case len(sizeStr) >= 1 && sizeStr[len(sizeStr)-1:] == "B":
		var size int64
		if _, err := fmt.Sscanf(sizeStr, "%dB", &size); err != nil {
			return 0, err
		}
		return size, nil
...
func parseDuration(durStr string) (int, error) {
	if durStr == "" {
	...
	switch {
	case len(durStr) >= 1 && durStr[len(durStr)-1:] == "s":
		var dur int
		if _, err := fmt.Sscanf(durStr, "%ds", &dur); err != nil {
			return 0, err
		}
		return dur, nil
		
	case len(durStr) >= 1 && durStr[len(durStr)-1:] == "m":
		var dur int
		if _, err := fmt.Sscanf(durStr, "%dm", &dur); err != nil {
			return 0, err
		}
		return dur * 60, nil
		
	case len(durStr) >= 1 && durStr[len(durStr)-1:] == "h":
		var dur int
		if _, err := fmt.Sscanf(durStr, "%dh", &dur); err != nil {
			return 0, err
		}
		return dur * 3600, nil
	...
	default:
		...
		var dur int
		if _, err := fmt.Sscanf(durStr, "%d", &dur); err != nil {
